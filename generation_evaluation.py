...
# Begin of your code
sample_op = lambda x : sample_from_discretized_mix_logistic(x, 1)
...
if __name__ == "__main__":
    ...
    model = PixelCNN(nr_resnet=1, nr_filters=1, input_channels=3, nr_logistic_mix=1)
    model = model.to(device)
    model.load_state_dict(torch.load('models/conditional_pixelcnn_min_depth.pth'))
    model = model.eval()
